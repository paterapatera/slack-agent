...
## Semche 検索の利用ポリシー（system prompt 反映）

- 内部の社内業務情報（要件・仕様・社内コード・社内ドキュメント等）に関する質問のときに検索ツールの利用を検討する。
- 外部公開情報や一般的/基礎的な質問では、検索を行わず LLM の知識で直接回答する。
- 検索ツールを使う場合は、原則として全文取得を推奨する。
  - include_documents=True
  - max_content_length=None
- 取得した本文は、回答時に必要箇所を引用・要約して提示する（コード側での強制トリムは行わない）。
